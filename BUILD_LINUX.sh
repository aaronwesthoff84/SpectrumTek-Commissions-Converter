...
pyinstaller build/spectrumtek_linux.spec --clean --noconfirm --workpath=build_temp --distpath=dist 2>&1
...
EXE_PATH="$PROJECT_ROOT/dist/SpectrumTek_Commissions_Converter"
...
if [ -f "$EXE_PATH" ]; then
    ...
    echo ""
    echo "  You can now:"
    echo "    - Run it directly: ./dist/SpectrumTek_Commissions_Converter"
    ...
    if [[ "$OPEN_FOLDER" == "y" || "$OPEN_FOLDER" == "Y" ]]; then
        if command -v xdg-open &> /dev/null; then
            xdg-open "$PROJECT_ROOT/dist" 2>/dev/null &
        elif command -v nautilus &> /dev/null; then
            nautilus "$PROJECT_ROOT/dist" 2>/dev/null &
        fi
    fi
else
